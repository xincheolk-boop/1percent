...
var AH = { "Access-Control-Allow-Origin": "*", "Content-Type": "application/json" };
var ajson = /* @__PURE__ */ __name((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: AH }), "ajson");
async function handleAdmin(request, env, url) {
  ...
  try {
    if (path === "/stats" && request.method === "GET") {
      const curMonth = (/* @__PURE__ */ new Date()).toISOString().slice(0, 7);
    ...
    if (path === "/users") {
      if (request.method === "GET") {
        ...
        if (q) {
          w.push("(u.uid LIKE ? OR u.nickname LIKE ?)");
          p.push(`%${q}%`, `%${q}%`);
        }
      ...
      if (request.method === "POST") {
        ...
        ).bind(b.nickname || "", b.uid || "", b.exchange || "", b.telegram || "", b.join_date || (/* @__PURE__ */ new Date()).toISOString().slice(0, 10), b.status || "active").run();
    ...
    if (uIdM) {
      ...
      if (request.method === "PUT") {
        const b = await request.json();
        await db.prepare("UPDATE users SET nickname=?,uid=?,exchange=?,telegram=?,join_date=?,status=? WHERE id=?").bind(b.nickname || "", b.uid || "", b.exchange || "", b.telegram || "", b.join_date || "", b.status || "active", id).run();
    ...
    if (sIdM) {
      ...
      if (request.method === "PUT") {
        const b = await request.json();
        await db.prepare("UPDATE settlements SET volume=?,fee=?,payback_amount=?,status=?,paid_date=? WHERE id=?").bind(b.volume || 0, b.fee || 0, b.payback_amount || 0, b.status || "pending", b.paid_date || null, id).run();
        return ajson({ ok: true });
      }
      if (request.method === "DELETE") {
    ...
    if (eIdM) {
      ...
      if (request.method === "PUT") {
        const b = await request.json();
        await db.prepare("UPDATE events SET exchange=?,title=?,description=?,image_url=?,link=?,start_date=?,end_date=?,is_active=? WHERE id=?").bind(b.exchange || "", b.title || "", b.description || "", b.image_url || "", b.link || "", b.start_date || "", b.end_date || "", b.is_active ? 1 : 0, id).run();
        return ajson({ ok: true });
      }
      if (request.method === "DELETE") {
    ...
    if (bIdM) {
      ...
      if (request.method === "PUT") {
        const b = await request.json();
        await db.prepare("UPDATE banners SET title=?,image_url=?,link=?,position=?,is_active=? WHERE id=?").bind(b.title || "", b.image_url || "", b.link || "", b.position || "main", b.is_active ? 1 : 0, id).run();
        return ajson({ ok: true });
      }
      if (request.method === "DELETE") {
    ...
    if (path === "/notices") {
      ...
      if (request.method === "POST") {
        const b = await request.json();
        const r = await db.prepare("INSERT INTO notices (title,content,is_pinned) VALUES (?,?,?)").bind(b.title || "", b.content || "", b.is_pinned ? 1 : 0).run();
        return ajson({ ok: true, id: r.meta.last_row_id });
      }
    }
    ...
    if (nIdM) {
      ...
      if (request.method === "PUT") {
        const b = await request.json();
        await db.prepare("UPDATE notices SET title=?,content=?,is_pinned=? WHERE id=?").bind(b.title || "", b.content || "", b.is_pinned ? 1 : 0, id).run();
        return ajson({ ok: true });
      }
      if (request.method === "DELETE") {
...
var REG_CORS = { "Access-Control-Allow-Origin": "*", "Content-Type": "application/json" };
var rjson = /* @__PURE__ */ __name((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: REG_CORS }), "rjson");
async function handleRegister(request, env) {
  ...
  try {
    ...
    return rjson({ error: "\uC798\uBABB\uB41C \uC694\uCCAD \uD615\uC2DD" }, 400);
  }
...
__name(getAccountFromRequest, "getAccountFromRequest");
var AH2 = { "Access-Control-Allow-Origin": "*", "Content-Type": "application/json" };
var aj2 = /* @__PURE__ */ __name((d, s = 200) => new Response(JSON.stringify(d), { status: s, headers: AH2 }), "aj2");
async function handleAuth(request, env, url) {
  ...
  if (path === "/signup" && request.method === "POST") {
    ...
    try {
      ...
      await db.prepare("INSERT INTO sessions (account_id, token, expires_at) VALUES (?,?,?)").bind(result.meta.last_row_id, token, expiresAt).run();
  ...
  if (path === "/login" && request.method === "POST") {
    ...
    try {
      ...
      const token = generateToken();
      const expiresAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1e3).toISOString();
      await db.prepare("INSERT INTO sessions (account_id, token, expires_at) VALUES (?,?,?)").bind(account.id, token, expiresAt).run();
...
__name(handleMypage, "handleMypage");
var CAL_CORS = {
  "Access-Control-Allow-Origin": "*",
